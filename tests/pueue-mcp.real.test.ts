...
const binaryName = process.platform === "win32" ? "pueue-mcp-real-test.exe" : "pueue-mcp-real-test";
...
function shellEchoCommand(message: string) {
  return process.platform === "win32" ? `cmd /c echo ${message}` : `sh -lc 'echo ${message}'`;
}

function shellExitCommand(exitCode: number) {
  return process.platform === "win32" ? `cmd /c exit ${exitCode}` : `sh -lc 'exit ${exitCode}'`;
}
